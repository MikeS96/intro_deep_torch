...
The goal is to minimize the Error function given by Eq. \eqref{eq:15}. Now if there are more than two classes, the only change would be add the additional summation as shown by Eq. \eqref{eq:12} to the error function. The Error function for more than one class is given as follow Eq. \eqref{eq:16}.

\begin{equation}
\label{eq:16}
E(W,b) = - \frac{1}{m} \sum_{i=1}^m \sum_{j=1}^n y_{ij}\ln{(\sigma(Wx^{(i)} + b))} 
\end{equation} 
